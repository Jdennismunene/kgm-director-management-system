...
const getDocumentType = (mimeType: string): "PDF" | "JPG" | "PNG" | null => {
  switch (mimeType) {
...
const getDocumentCategory = (category: string): DocumentCategory | null => {
...
const deletePhysicalFile = (filePath: string | null | undefined): void => {
  if (!filePath) {
    return;
  }
...
export const createDocument = async (
  ...
  try {
    ...
    if (!type) {
      ...
      res.status(400).json({
        message:
          "Unsupported document type. Only PDF, JPG and PNG are allowed.",
      });
      return;
    }
  ...
  try {
    ...
    if (req.file) {
      ...
      updateData.fileUrl = `${req.protocol}://${req.get(
        "host",
      )}/uploads/documents/${newFile.filename}`;
