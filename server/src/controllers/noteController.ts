 ...
 */
const validNoteTypes = ["GENERAL", "PRAYER", "PROGRESS", "FOLLOW_UP"] as const;

type NoteType = (typeof validNoteTypes)[number];

/**
 * Check whether a value is a valid NoteType.
 */
const isValidNoteType = (value: unknown): value is NoteType => {
  return (
    typeof value === "string" && validNoteTypes.includes(value as NoteType)
  );
};
...
export const createNote = async (req: Request, res: Response) => {
  try {
    ...
    if (type !== undefined && !isValidNoteType(type)) {
      return res.status(400).json({
        message:
          "Invalid note type. Allowed values: GENERAL, PRAYER, PROGRESS, FOLLOW_UP",
      });
    }
